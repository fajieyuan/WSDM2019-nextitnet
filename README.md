...
To be specific, they use session length (by a sliding window) 10 for GRU4Rec and other baselines, but using 500 for NextItNet (the authors performed experiments exactly in this way), because the longest user session is 500 in the dataset. However, please note that in our paper (see Section 4.1.1) we have splitted the long-range user session into several sub-sessions and compare all methods under the same session length for fair comparison. This has a very large impact on the final results regarding both accuracy and training time.
For example, if users have 50 positive interactions in average, the former preprocessing method will create 5 training sequences (ignoring data augmentation, see Section 3.1) while for NextItNet there is only one training sequence. In other words, the number of training examples for baselines are 5 times larger than NextItNet. In terms of training time, it is even much worse since padding around 450 zeros for each user will of course make the training extremely slow.  In other words, the evaluation in this paper is very unfair and meaningless, resulting misleading conclusions. 
